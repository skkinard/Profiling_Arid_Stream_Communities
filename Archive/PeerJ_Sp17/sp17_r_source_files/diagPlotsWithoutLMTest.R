...
## Currently, it supports lm, aov, gls, and lme models
...
get.r <- function(model.class, model) {
  ...
  if (model.class=="gls" | model.class=="lme") {
...
plot.continuous <-function(f, r, f.name, r.name, model.class, model=NULL) {
  f <- round(f, 6)
  if (!is.null(model) & (model.class=="lm" | model.class=="aov")) {
    bp.p.value <- bptest.bsb(model)$p.value
  }
  else {
    whee <- lm(r~f)
    bp.p.value <- bptest.bsb(whee)$p.value
  }
...
diag.plots <- function(model, col.nos=c(), data=NULL, std=T, force.RvF=F) {
  if (length(col.nos)>0 & is.null(data)) {
    ...
    cat("\n")
    cat("diag.plots(my.model, data=my.data, col.nos=c(2, 4)")
    cat("\n"); cat("\n")
    return()
  }
  ...
  if (std) {
    f <- fitted(model)
    if (!is.ANOVA | force.RvF) {plot.continuous(f, r, "Fitted", "Residuals", model.class, model)}
#    print("here")
    qqnorm.with.sim.bounds(r)
    if (model.class=="lm" | model.class=="aov") {plot(model, which=4)}
  }
  ...
  if (length(col.nos) > 0) {
    for (j in col.nos) {
      ...
      else {plot.continuous(data[,j], r, colnames(data)[j], model.class, "Residuals")}
    }
  }
}

## The Breusch-Pagan test without the lmtest package

bptest.bsb <- function(model, studentize=TRUE) {
  X <- model.matrix(model)
  sigma.hat.sq <- sum(resid(model)^2)/nrow(X)
  g <- resid(model)^2/sigma.hat.sq
  g.beta <- solve(t(X)%*%X)%*%t(X)%*%g
  g.r <- g - X%*%g.beta
  BP <- (sum((g-1)^2) - sum(g.r^2))/2
  if (!studentize) {return(list(BP=BP, p.value=pchisq(BP, ncol(X)-1, lower.tail=F)))}
  w <- resid(model)^2 - sigma.hat.sq
  BP <- BP*2*sigma.hat.sq^2/mean(w^2)
  return(list(BP=BP, p.value=pchisq(BP, ncol(X)-1, lower.tail=F)))
}
