h1 <- importDVs(staid = "08212300", code = "00060", stat = "00003", sdate = "2002-01-01",edate = as.Date(Sys.Date(), format = "%Y-%m-%d"))
All <- h1[,c(2,3)]
...
names(All) <- c("dates", "TRC", "OSO", "AR", "MC", "PDC", "CC", "MR", "GC", "PLC", "BC", "BB", "PB")
data2 <- reshape(All,varying= c("TRC", "OSO", "AR", "MC", "PDC", "CC", "MR", "GC", "PLC", "BC", "BB", "PB"), v.names = "Discharge",timevar = "Site", times = c("TRC", "OSO", "AR", "MC", "PDC", "CC", "MR", "GC", "PLC", "BC", "BB", "PB"), direction = "long")
...
sites_na <- c(1,5,6,7,10,12)
for (x in sites_na) {
  metrics.c[x,] <- metrics.17[x,]
