...
fish.model.9 <- lm(Shannon.Index~Log.NH3, data=fish2.lm.1)
summary(fish.model.9)
vif(fish.model.9)

fish.model.49 <- lm(Shannon.Index~Log.NO3+Log.PPT, data=fish2.lm.1)
summary(fish.model.49)
vif(fish.model.49)
##############################################################################
################################################################################
...
summary(Model.5.InvertDredge)

Model.3.InvertDredge <- lm(SID~AvgPPT, data=Inv_model)
summary(Model.3.InvertDredge)

#Finalized aug pair plots
FishAugPairs <- read.csv("C:/Users/Sean/Desktop/FishAugPairs.csv")
source('C:/Users/Sean/Downloads/augPairs (1).R', encoding = 'UTF-8')
aug.pairs(FishAugPairs)

InvAugPairs <- read.csv("C:/Users/Sean/Desktop/LimnoData/Inv.model.csv")

aug.pairs(InvAugPairs)