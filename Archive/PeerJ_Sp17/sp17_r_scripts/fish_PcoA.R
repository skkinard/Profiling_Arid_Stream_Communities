# fish Principle coordinate analysis
# Sean Kinard
# 12-17-2020
...
env <- env[,c(which(  colnames(env) == "STAID" |
                        colnames(env) == "AP" |
                        colnames(env) == "canopy" |
                        colnames(env) == "bank.height" |
                        colnames(env) == "NH4."|
                        colnames(env) == "Soil.Org"|
                        colnames(env) == "Bas.forest"|
                        colnames(env) == "Rip.forest"
))]
...
## first, calculate the distance matrix using the Bray-Curtis distance.
fish_dist <- vegdist(fish[,-1], method="bray")

## do the MDS math (this is basically eigen value decomposition)
fish_pcoa <- cmdscale(fish_dist, eig=TRUE, x.ret=TRUE)

## calculate the percentage of variation that each MDS axis accounts for...
fish_percent <- round(fish_pcoa$eig/sum(fish_pcoa$eig)*100, 1)

## now make a fancy looking plot that shows the MDS axes and the variation:
mds.values <- fish_pcoa$points
mds.data <- data.frame(Sample=fish[,1],
                       X=mds.values[,1],
                       Y=mds.values[,2])
mds.data
...
ggplot(data=mds.data, aes(x=X, y=Y, label=Sample)) +
  geom_text() +
  theme_bw() +
  xlab(paste("MDS1 - ", fish_percent[1], "%", sep="")) +
  ylab(paste("MDS2 - ", fish_percent[2], "%", sep="")) +
  ggtitle("PCoA of fish community using bray-curtis distance")
...
# creating a merged dataframe with site lable, 7 environmental variables, and 18 species
mstercom <- merge(env, fish, by = "STAID")

# set variables
evars <- c(2:(length(colnames(env))))
cvars <- c((1+length(colnames(env))):length(colnames(mstercom)))

# scale environmental variables
mstercom[,c(evars)] <- scale(mstercom[,c(evars)])

varespec.bray <- vegdist(mstercom[,c(cvars)], method = "bray") # dissimilarity matrix using bray-curtis distance indices on the varespec dataset native to vegan
...
# Plot PcoA
pvec <- as.data.frame(pcoaVS$vectors)
ggplot(pvec, aes(x=V1, y=V2)) + geom_point()
...
# constrained to precipitation PcoA
