...
par(mfrow=c(2,2))
plot(shannon ~ PO43., xlab="Ammonia Concentration (ppm)", ylab="Shannon Index", data=trm.invert)
abline(lm(shannon ~ PO43., trm.invert))
summary(lm(shannon ~ PO43., trm.invert))
# p-value: 0.1968

plot(rare.rich ~ PO43., xlab="Ammonia Concentration (ppm)", ylab="Rarified Richness", data=trm.invert)
abline(lm(rare.rich ~ PO43., trm.invert))
summary(lm(rare.rich ~ PO43., trm.invert))
#  p-value: 0.2339

plot(shannon ~ HYDRO_DISTURB_INDX, xlab="Ammonia Concentration (ppm)", ylab="Shannon Index", data=trm.invert)
abline(lm(shannon ~ HYDRO_DISTURB_INDX, trm.invert))
summary(lm(shannon ~ HYDRO_DISTURB_INDX, trm.invert))
# p-value: 0.05573 

plot(rare.rich ~ HYDRO_DISTURB_INDX, xlab="Ammonia Concentration (ppm)", ylab="Rarified Richness", data=trm.invert)
abline(lm(rare.rich ~ HYDRO_DISTURB_INDX, trm.invert))
summary(lm(rare.rich ~ HYDRO_DISTURB_INDX, trm.invert))
# p-value: 0.06983
...
ford <- ford[order(ford$PPTAVG_BASIN),]
ford$Col <- rbPal(10)[as.numeric(cut(ford$PPTAVG_BASIN,breaks = 10))]
plot(nms1, disp = "sites", type = "n", main="nmds.invert", ylim=c(-1,1), xlim=c(-1.5,2)) # Why won't my x and y axis adjust? Is it because my aspect ratio is locked? How do I change aspect ratio?
ordiellipse(nms1, groups = ford$clusterCut, draw="polygon", border = FALSE, kind = "ehull", col=c("yellow", "brown", "blue"), alpha=.2)
